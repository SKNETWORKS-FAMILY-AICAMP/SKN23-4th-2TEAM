...
### 3. 관리자 페이지 챗봇 도입 및 사용자 / 관리자 페이지의 API 이원화
...
이전 프로젝트가 Streamlit 기반으로 빠른 프로토타이핑과 기능 검증에 강점이 있었다면, 최신 프로젝트는 배포를 가정한 서비스형 구조에 맞추어 React를 도입했다.

이를 통해 관리자 페이지와 작업자 페이지를 역할에 맞게 분리하고, 더 정교한 상태 관리와 실시간 상호작용, 반응형 UI를 유연하게 구현할 수 있게 되었으며, 단순 데모를 넘어 실제 서비스 배포에 적합한 프론트엔드 구조로 발전시켰다.

---

### 5. 공장 환경을 고려한 터치형 작업자 UI 개선

이전 프로젝트의 사용자 페이지는 일반적인 채팅 UI에 가까워 작업자가 직접 문장을 타이핑해야 하는 구조였다면, 최신 프로젝트는 실제 공장 현장을 주요 사용 환경으로 가정해 입력 방식 자체를 단순화했다.

특히 소음이 크고 장갑을 착용한 상태에서 긴 문장을 입력하기 어렵다는 점을 반영하여, 터치 기반 인터페이스와 최소 입력 흐름을 적용했으며, 작업자는 복잡한 설명 없이도 에러코드만 입력하면 즉시 AI의 진단과 대응 가이드를 받을 수 있도록 개선했다.

---

### 6. Docker 도입을 통한 안정적인 배포
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td valign="top">Django를 맡게 되면서 수업 진도보다 먼저 프로젝트를 시작하게 되었고, 스스로 예습하며 구현해야 하는 도전적인 상황이었습니다. 처음 접하는 프레임워크를 실전 프로젝트에 적용하며 시행착오도 많았지만, 문제를 하나씩 해결해 나가며 결국 프로젝트를 마무리할 수 있었습니다. 비록 완성도 측면에서는 아쉬움이 남지만, Django의 전반적인 구조와 흐름을 직접 경험하며 이해할 수 있었고, 실무와 유사한 개발 과정을 겪어볼 수 있었다는 점에서 의미 있는 시간이었습니다.<br>이번 경험을 바탕으로 부족했던 부분을 보완하고 보다 안정적이고 완성도 높은 서비스를 구현할 수 있도록 노력할 것입니다.</td>
    </tr>
    <tr>
      <td valign="center">신승훈</td>
      <td valign="top">RAG 기반 AI 백엔드의 검색·진단·응답 흐름을 분석하며, 서비스가 단순 기능 단위가 아니라 검색 → 문맥 구성 → 응답 생성이 유기적으로 연결된 구조임을 체감했습니다. 특히 BM25와 reranker를 직접 수정하고 연결하는 과정에서, 검색 품질이 단순 알고리즘 선택이 아니라 데이터 흐름과 파이프라인 구조에 크게 의존한다는 점을 깨달았습니다. 또한 검색 로직을 pipeline으로 분리하면서 기능 구현보다 구조 설계가 유지보수성과 확장성에 더 큰 영향을 준다는 것을 느꼈습니다. 이 과정에서 일부 동기식 처리 구조가 외부 모델 호출 및 검색 단계에서 병목을 유발해 전체 응답 지연으로 이어질 수 있음을 체감했고, 이를 비동기 처리로 전환해야 하는 필요성을 이해하게 되었습니다.</td>
    </tr>
    <tr>
