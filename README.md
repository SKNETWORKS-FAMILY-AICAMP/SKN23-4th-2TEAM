...
| PM(일정/작업 조율), `worker UI` 핵심 화면 구현, 공통 유틸리티 수립                                                                     | Worker UI(FastAPI) 인프라 구축, PostgreSQL & Vector DB 연동, CORS 및 통신 보안 정책 수립                                                         | `backend_hub` (Django) 서버 개발, 비즈니스 데이터 모델링(ORM), 통계 및 Admin용 CRUD API 구현                                                           | `LangGraph` 기반 AI 워크플로우 설계, `bge-reranker` 적용 및 검색 품질 고도화, 전처리 파이프라인                                                   | `admin UI` 관리자 관제 센터 구축, 데이터 시각화, 실시간 모니터링 UI 구현                                                                             |
...
<details>
<summary><b>테스트 계획 및 결과 상세 보기 (클릭하여 펼치기)</b></summary>
<br>
...
**잘 작동하는 항목**
- 사용자 페이지는 100건 중 99건에서 매뉴얼과 응답이 정상적으로 매칭되었습니다.
- 사용자 페이지 체크리스트는 대부분 5개 항목으로 안정적으로 생성되었고, 중복/빈 항목이 발생하지 않았습니다.
- 관리자 페이지는 환각 억제 지표가 0.916으로 비교적 안정적이었고, 금지 키워드 오탐은 발생하지 않았습니다.
- 관리자 페이지의 라인별 건수 질의, 일부 에러 카운트 질의는 정확하게 처리되었습니다.

**개선 필요 항목**
- 사용자 페이지는 매뉴얼이 존재하지 않는 에러코드(`0X05`)에서 체크리스트가 생성되지 않았습니다.
- 사용자 페이지는 일부 UR 계열 케이스에서 핵심 조치 항목 누락으로 checklist score가 0.8 수준에 머무는 사례가 있었습니다.
- 관리자 페이지는 `top_error` 질문을 `error_list`로 잘못 분류하는 경향이 확인되었습니다.
- 관리자 페이지는 `unresolved_list`, `error_list`, `line_risk`, `error_code_analysis` 일부 케이스에서 운영 데이터 근거가 부족하거나 핵심 포인트가 누락되었습니다.
- 관리자 페이지 오답 16건 중 주요 실패 유형은 `error_list` 4건, `unresolved_list` 4건, `top_error` 2건, `error_code_analysis` 2건이었습니다.
...
| 평가 도구 | LangSmith |
| 테스트 일자 | 2026-03-19 |
| 사용자 페이지 평가 파일 | `worker_eval_20260319_215959.csv` |
| 관리자 페이지 평가 파일 | `manager_eval_20260319_225233.csv` |
| 평가 방식 | CSV 결과 기반 정량/정성 분석 |
...
- 프로젝트 피벗(Pivot)에 따른 일정 압박 및 소통 병목  
  텍스트 챗봇에서 `Touch UX 기반 상용 HMI`로 기획이 전면 수정되며, 한정된 기간 내 FE-BE(Django)-AI(FastAPI) 3개 파트 간 API 연동과 소통 병목이 발생  
  해결: 데일리 스크럼을 도입해 파트별 진행 상황을 투명하게 동기화하고, 프론트엔드와 백엔드가 병렬로 작업할 수 있도록 요청/응답 JSON 기반 API 스펙을 최우선으로 확정해 개발 속도를 크게 개선

# 10. 회고

| 이름   | 회고                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 송주엽 | 이번 프로젝트에서 PM과 FE 역할을 함께 맡아 일정 관리와 화면 구현을 병행했습니다. PM으로서 결과물의 완성도를 더 높였으면 좋았겠다는 아쉬움은 남지만, 팀원별 진행 상황을 맞추고 API 연동 시점을 조율하는 과정에서 끝까지 책임감을 갖고 최선을 다했습니다. 요구사항을 기능 단위로 나눠 우선순위를 정리하며 프로젝트를 안정적으로 마무리했고, 작업자 UI와 관리자 UI를 정리하면서 사용자 관점에서 "즉시 이해되는 흐름"의 중요성을 깊이 배웠습니다. 다음 프로젝트에서는 초기 설계 단계부터 화면 구조와 API 계약을 더 정교하게 맞춰 개발 효율과 품질을 함께 끌어올리고 싶습니다.                                                                                                                                              |
| 김도영 | 아아아아아                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 김민정 | 아아아아아                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 신승훈 | 아아아아아                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
