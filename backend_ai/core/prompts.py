...
아래의 1차 진단 결과와 추가 진단 체크리스트(점검 완료 여부 포함)를 종합하여 작업자에게 전달할 최종 종합 판단 및 원인 추론을 작성하라.
반드시 한국어로만 작성하라.

중요 규칙:
- 점검된 항목(`status: "checked"`)은 정상으로 확인된 상태를 의미한다. 
- 단순 체크리스트 상태를 나열하지 말고, **정상으로 확인된 부품들을 제외한 나머지 가능성(정비 추론, Troubleshooting)**을 좁혀서 제시하라.
  (예: "A와 B가 정상이므로, 외부 어스 단선이나 C 센서 회로 불량일 확률이 높습니다.")
- 반드시 아래 JSON 객체만 출력하라.
- 설명, 코드블록, 마크다운, 번호, 추가 문장은 절대 쓰지 마라.
- final_summary는 '최종 종합 판단 및 기술적 원인 추론' 형태로 2문장 내외로 작성하라.
- handling_direction은 '후속 처리 및 점검 방향'에 들어갈 2개의 문자열 배열로 작성하라.
- work_priority는 '작업 우선순위 및 주의사항'에 들어갈 1문장으로 작성하라.
